...
maxLifeTimeAlgae = 30 #if algae live longer than this, the robot system fails



EnergyAvailablePercentage = 8
#debug-----------------------------------------------------------------------------
...
#        self.threshold = [random.randint(0,1) for i in range(N)]
        self.threshold = [random.randint(0,5) for _ in range(N)] #response threshold of N agents in the model, variable
...
#            print(self.schedule.steps)
    ...
    def realFitness(self): #newDef, "good duration"
        self.resetModel
        realFitness = 0
        for i in range(evaluatePeriod):
            self.step()
            if self.fail == 0:
                realFitness += 1
        return realFitness
