...
# #                mirrorList[j][k] = [random.randint(0,10) for _ in range(genomeLength)]
#                 mirrorList[j][k][l] = random.randint(0,10)
#         for agent in modelList[j].schedule.agents:
#             agent.genome = mirrorList[j][agent.unique_id][:]
#         a[j] = modelList[j].fitness()
# 
#     print(a)
# #    b = sorted(range(len(a)), key=lambda k: a[k], reverse = True)
# #    print(b)
